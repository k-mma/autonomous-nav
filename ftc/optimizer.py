...
#
# All four non-corridor profiles below run at fidelity="realistic",
# unlike DEFAULT_PROFILES' first three (which stay on the "optimistic"
# default so they keep reproducing ftc_suite_writeup.md's published
# numbers -- see ftc/config.py's MODEL_FIDELITY comment). "optimistic"
# isn't a neutral default: it models an omnidirectional 360-degree
# camera and zero heading drift, neither of which any real webcam has
# (ftc/config.py's CAMERA_FOV_DEG_BY_TIER cites 60-78 degrees across
# every AprilTag-capable camera FTC's own docs list). Under that
# assumption a second camera mount or the free IMU literally cannot
# change a single trial's outcome -- a front-only and a front+rear
# camera suite are trial-for-trial identical, because one omnidirectional
# camera already sees every direction the second one could add. Since
# this catalog's whole purpose is "what a team would actually face," it
# shouldn't also assume a camera hardware nothing on the shelf has.
# Switching to "realistic" (60-degree FOV, real heading drift) was
# checked, not assumed, against a concern that it might reopen the same
# trap DEFAULT_PROFILES' combined_realistic falls into (see above): at
# 100 trials every one of the four profiles below still separates best
# single from best bundle (+2 to +7 points) at this fidelity, AND the
# rear camera and IMU stop being invisible -- dual_camera_apriltag
# starts beating a single camera on its own merits instead of tying it
# exactly. corridor is left on its DEFAULT_PROFILES default (also
# "optimistic") because it's the same shared object DEFAULT_PROFILES
# uses -- giving it a different fidelity here would silently change
# DEFAULT_PROFILES' own corridor scenario, and it doesn't need the
# change anyway: it already separates single from bundle by +24 points.
MATCH_PROFILES = [
    ScenarioProfile("pose_drift_match", "Heavy pose drift (map also off)", variance_level=0.5,
                    start_drift_scale=1.0, obstacle_drift_scale=0.5, fidelity="realistic"),
    ScenarioProfile("map_error_match", "Field doesn't match the map (also drifting)", variance_level=0.5,
                    obstacle_drift_scale=1.0, start_drift_scale=0.5, fidelity="realistic"),
    ScenarioProfile("opponent_match", "Opponent parks in the route (also drifting)", variance_level=0.5,
                    blocker_scale=1.0, start_drift_scale=0.5, fidelity="realistic"),
