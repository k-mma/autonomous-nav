...
| AprilTag max correction | AprilTag (front camera) | 0.25x | Odometry pods | yes |
...
The recommendation is not universally robust: AprilTag max correction tips the best-value suite at only 0.25x its estimated value -- the least forgiving parameter this sweep found. See the table above for the rest; any row with a real (non-noise) tip is a specific, named number a reviewer can push back on, which is the point of running this at all.
...
Parameters that never tipped the ranking anywhere in [0.25x, 4x]: Odometry-pod drift rate (ODOMETRY_DRIFT_PER_CELL), AprilTag range+angle degradation, Distance sensors cost, Full suite cost.
