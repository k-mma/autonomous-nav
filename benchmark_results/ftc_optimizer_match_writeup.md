...
Every buildable combination of 7 sensor suites (ftc/sensors.py), up to 3 suites per bundle, composed by ftc/bundle.py and evaluated by ftc/optimizer.py over 5 scenario profiles x 90 seeded trials each. 63 raw combinations collapse to 23 distinct robots once bundles that buy identical hardware are recognized as the same purchase (10350 matches). Raw data in `ftc_optimizer_match_results.csv`, charts in `ftc_optimizer_match_frontier.png` and `ftc_optimizer_match_synergy.png`.
...
|---:|---|---:|---:|---:|---:|
| 1 | odometry pods + front camera + rear camera | $244.85 | 26% | 12% | +6.3 |
| 2 | odometry pods + IMU + front camera + rear camera | $244.85 | 26% | 12% | +6.3 |
| 3 | odometry pods + front camera | $219.85 | 22% | 10% | +5.6 |
| 4 | odometry pods + IMU + front camera | $219.85 | 22% | 10% | +5.6 |
| 5 | front camera + rear camera | $50.00 | 16% | 9% | +10.7 |
| 6 | IMU + front camera + rear camera | $50.00 | 15% | 7% | +9.3 |
| 7 | odometry pods + front camera + rear camera + 3 ToF sensors | $339.35 | 15% | 9% | +1.3 |
| 8 | front camera | $25.00 | 14% | 7% | +16.9 |
| 9 | IMU + front camera | $25.00 | 14% | 6% | +16.0 |
| 10 | odometry pods + front camera + 3 ToF sensors | $314.35 | 14% | 8% | +1.2 |

Best average: odometry pods + front camera + rear camera ($244.85, 26%). Most robust -- highest success rate on its OWN WORST scenario, the right objective if you can't predict your division: odometry pods + front camera + rear camera ($244.85, 12% worst-case vs. 12% for the best-average robot). The two objectives pick the literal SAME robot here, not just a tie on worst-case rate -- there is no best-average-vs-most-robust tradeoff to report in this catalog, and a team should read that as "the choice was easy," not as a coincidence worth distrusting.
...
|---|---|---:|---:|---:|---|---:|---:|---|
| Odometry pods + Rear camera | Rear camera | 26% | 16% | +10.0% | [+7.1%, +12.9%] | 0.000 | $+194.85 | **significant** |
| Odometry pods + IMU + Rear camera | Rear camera | 26% | 16% | +10.0% | [+7.1%, +13.1%] | 0.000 | $+194.85 | **significant** |
| Odometry pods + AprilTag (front camera) | AprilTag (front camera) | 22% | 14% | +8.0% | [+5.3%, +10.9%] | 0.000 | $+194.85 | **significant** |
| Odometry pods + AprilTag + IMU | AprilTag + IMU | 22% | 14% | +8.2% | [+5.6%, +11.3%] | 0.000 | $+194.85 | **significant** |
| IMU + Rear camera | Rear camera | 15% | 16% | -0.7% | [-1.8%, +0.2%] | 0.954 | $+0.00 | no gain |
| Odometry pods + Distance sensors + Rear camera | Rear camera | 15% | 16% | -0.9% | [-5.1%, +3.1%] | 0.675 | $+289.35 | no gain |
| Odometry pods + Distance sensors + AprilTag (front camera) | AprilTag (front camera) | 14% | 14% | -0.4% | [-4.2%, +3.6%] | 0.604 | $+289.35 | no gain |
| Odometry pods + Distance sensors + AprilTag + IMU | AprilTag + IMU | 14% | 14% | -0.2% | [-4.0%, +3.8%] | 0.581 | $+289.35 | no gain |

4 of the 8 bundles tested beat their own best single component by a statistically significant margin. The cheapest of those upgrades is Odometry pods + Rear camera: +$194.85 over Rear camera for +10.0% success rate [95% CI +7.1%, +12.9%].

By marginal value -- gain per dollar of the UPGRADE, not of the whole robot -- the best combination to buy is Odometry pods + Rear camera at +5.1pp per extra $100 over Rear camera alone.
...
|---|---:|---:|---:|---:|
| encoders only | $0.00 | 10% | 7% | 0 |
| front camera | $25.00 | 14% | 7% | 1 |
| front camera + rear camera | $50.00 | 16% | 9% | 2 |
| odometry pods + front camera | $219.85 | 22% | 10% | 2 |
| odometry pods + front camera + rear camera | $244.85 | 26% | 12% | 3 |

### Best robot at each budget

| Budget | Best robot | Cost | Money left over | Weighted success |
|---:|---|---:|---:|---:|
| $50 | front camera + rear camera | $50.00 | $0.00 | 16% |
| $150 | front camera + rear camera | $50.00 | $100.00 | 16% |
| $300 | odometry pods + front camera + rear camera | $244.85 | $55.15 | 26% |
| $500 | odometry pods + front camera + rear camera | $244.85 | $255.15 | 26% |
...
|---|---|---:|---:|
| Heavy pose drift (map also off) | odometry pods + front camera + rear camera | 12% | $244.85 |
| Field doesn't match the map (also drifting) | odometry pods + front camera + rear camera | 21% | $244.85 |
| Opponent parks in the route (also drifting) | odometry pods + front camera + rear camera | 17% | $244.85 |
| Everything at once (match-realistic mix) | odometry pods + front camera + rear camera | 16% | $244.85 |
| Tight corridor, mixed deviation | odometry pods + front camera | 62% | $219.85 |
...
|---:|---|---|---:|---:|---:|---|---:|---|
| 1 | (best single, the starting point) | front camera + rear camera | $50.00 | 16% | -- | -- | -- | -- |
| 2 | Odometry pods | odometry pods + front camera + rear camera | $244.85 | 26% | +10.0% | [+7.1%, +13.1%] | 0.000 | yes |
...
- The best single suite (Rear camera = front camera + rear camera, $50.00) reaches 16% against the best bundle's 26% at $244.85. Bundling buys +10% for $+194.85 -- read the per-dollar column before reading that as an endorsement.
